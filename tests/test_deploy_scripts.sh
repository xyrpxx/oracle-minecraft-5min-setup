...
assert_file_contains deploy/remote_provision.sh "required_java_major"          "Java requis résolu depuis les métadonnées Mojang"
assert_file_contains deploy/remote_provision.sh "STATE_FILE"                  "fichier d'état pour reprise"
assert_file_contains deploy/remote_provision.sh "mark_step"                   "helper de marquage des étapes"
assert_file_contains deploy/remote_provision.sh "is_step_done"                "skip des étapes déjà faites"
assert_file_contains deploy/remote_provision.sh "ensure_java"                  "installation Java adaptative"
assert_file_contains deploy/remote_provision.sh 'openjdk-${want}-jre-headless' "paquet openjdk adapté à la version MC"
assert_file_contains deploy/remote_provision.sh "update-alternatives --set java" "JDK par défaut ajusté si plusieurs installés"
assert_file_contains deploy/remote_provision.sh "/ZGenerational/d"             "retrait ZGenerational sur JDK 24+"
assert_file_contains deploy/remote_provision.sh "Forge le supporte-t-il"       "erreur claire si Forge indisponible pour la version"
assert_file_not_contains deploy/remote_provision.sh "openjdk-21-jre-headless"  "plus de version Java codée en dur"
...
assert_file_contains deploy/remote_provision.sh "loader.version"                "loader fabric résolu PAR version du jeu"
assert_file_contains deploy/remote_provision.sh "versions/installer"            "launcher fabric résolu dynamiquement"
assert_file_contains deploy/remote_provision.sh "/1.1.0/server/jar"            "repli launcher fabric épinglé (URL 3 segments)"
...
assert_file_contains deploy/remote_provision.sh "download_verified"            "téléchargement vérifié (sha1)"
assert_file_contains deploy/remote_provision.sh "downloads.server.sha1"        "sha1 Mojang officiel (vanilla fail-closed)"
assert_file_contains deploy/remote_provision.sh "installer}.sha1"              "sidecar .sha1 Maven (Forge)"
assert_file_contains deploy/remote_provision.sh "sha1sum -c"                   "vérification sha1 effective"
...
echo "-- crafty-seed.sh (API Crafty v2) --"
assert_file_exists deploy/crafty-seed.sh "crafty-seed.sh présent"
assert_exit_zero "syntaxe bash valide" bash -n deploy/crafty-seed.sh
assert_file_contains deploy/crafty-seed.sh "/api/v2/auth/login"    "login Crafty v2 (POST)"
assert_file_contains deploy/crafty-seed.sh "/api/v2/servers"      "CRUD serveurs Crafty v2"
assert_file_contains deploy/crafty-seed.sh "Bearer"                "auth Bearer (JWT)"
assert_file_contains deploy/crafty-seed.sh "download_jar"          "type serveur download_jar"
assert_file_contains deploy/crafty-seed.sh "ENV_FILE"              "lit le mot de passe Crafty depuis .env"
assert_file_contains deploy/crafty-seed.sh "crafty-seed.sh"        "deployé dans /opt/minecraft/bin/"
